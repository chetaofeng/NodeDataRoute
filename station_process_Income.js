...
var getFirstFile = function () {
  return new Promise(function (resolve, reject) {
      ...
      //获取第一个待传文件名称
      fs.readdir(config.local.station_income_path, function (err, files) {

          if (files.length > 0) {



            var filePath = path.normalize(config.local.station_income_path + files[0])

            pathto_000001 = path.normalize(config.local.station_mysql_path + files[0])
            pathto_000010 = path.normalize(config.local.sub_path + files[0])
            pathto_000100 = path.normalize(config.local.center_path + files[0])
...
var proceData = function () {
  ...
  return new Promise(function (resolve, reject) {
    ...
    fs.writeFileSync(pathto_000010, JSON.stringify(data), {flag: 'w', encoding: 'utf8'})
